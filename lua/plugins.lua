...
-- Copilot Chat
{
  ...
  { "akinsho/bufferline.nvim", config = function() require("bufferline").setup() end },
  {
    ...
    config = function()
      ...
      notify.setup({ stages = "fade_in_slide_out", timeout = 3000, background_colour = "#000000", render = "minimal" })
      vim.notify = notify
    end
  },
...
for _, server in ipairs(mason_lspconfig.get_installed_servers()) do
  ...
  { "numToStr/Comment.nvim", config = function() require("Comment").setup() end }, { "windwp/nvim-autopairs", event = "InsertEnter", config = function() require("nvim-autopairs").setup() end },
    ...
    elseif luasnip.jumpable(-1) then
      ...
      fallback()
        ...
        sources = cmp.config.sources({
            {name = "copilot"},{ name = "nvim_lsp" }, { name = "luasnip" }, { name = "buffer" }, { name = "path" },
        }),
      })
    end
  },
